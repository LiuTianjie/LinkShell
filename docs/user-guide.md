...
LinkShell 让你在手机上远程查看和控制电脑上的 Claude Code / Codex / Gemini / Copilot 终端会话，也可以用 Agent Workspace 直接操作 Claude Code 与 Codex 的结构化对话。
...
除了手机 App，任何 Gateway 都会在**自己的地址上**同源提供一个网页版控制台——和手机端是同一个 Agent + 终端界面。CLI 自带这个网页（内置网关也有），无需单独部署。
...
打开后默认连接的就是「提供这个页面的网关」（web 跟随 gateway）。你会看到该网关下的会话列表，点进任意会话即可进入 Agent 控制台或终端。
...
> 提示：网页也可以指向另一个网关——在页面里修改 Gateway 地址即可，配置会本地持久化（适合把本地打开的页面连到远程网关调试）。
...
linkshell start --daemon --provider claude --agent-ui --agent-provider claude
...
| 选项 | 说明 |
|------|------|
| `--provider <name>` | 终端 provider：`claude` / `codex` / `gemini` / `copilot` / `custom` |
| `--command <cmd>` | 覆盖 provider 命令；`custom` 必填 |
...
### Gemini / Copilot provider 找不到

`linkshell start --provider gemini` 和 `linkshell start --provider copilot` 会从 `PATH` 查找对应 CLI。如果你用 nvm、Homebrew 或自定义安装目录，先确认命令在当前 shell 中可执行：
