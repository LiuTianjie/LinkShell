...
## 18. 当前实现状态
当前仓库已完成第一轮工程初始化，目标是先把最小闭环的基础设施搭起来，而不是直接追求完整终端体验。

已完成内容：

1. 已建立 pnpm monorepo。
2. 已创建共享协议包，统一 envelope、session 和 terminal 事件模型。
3. 已创建 CLI 包，支持使用 PTY 启动本地 provider，并通过 WebSocket 连接网关。
4. 已创建网关服务，支持健康检查、内存态 pairing 和最小消息转发。
5. 已创建 Expo App 壳，支持 claim pairing、连接会话、展示终端文本和发送输入。

当前范围内的实现定位：

1. CLI、网关、App 已有可继续开发的最小骨架。
2. 网关当前使用内存态 session 和 pairing，尚未接入 Redis。
3. App 当前是终端文本镜像壳，不是完整终端渲染器。
4. CLI 当前支持最小 PTY 到 WebSocket 转发，尚未实现断线恢复、ACK 补发和 scrollback 重放。
...
1. apps/mobile
Expo App，当前提供 pairing claim、WebSocket 连接、文本输出展示和输入发送。

2. packages/shared-protocol
三端共享协议定义，避免消息结构漂移。

3. packages/cli
本地桥接 CLI，负责 PTY、provider 启动和网关连接。

4. packages/gateway
云端网关 MVP，负责 pairing、会话连接和 host/client 间消息转发。
