...
export async function loadHistory(): Promise<ConnectionRecord[]> {
  try {
    ...
    if (!raw) return [];
    const parsed = JSON.parse(raw) as HistoryEnvelope | ConnectionRecord[];
    // Versioned shape: only trust records when the version matches.
    if (parsed && !Array.isArray(parsed) && typeof parsed === "object") {
      if ((parsed as HistoryEnvelope).version !== HISTORY_VERSION) return [];
      const records = (parsed as HistoryEnvelope).records;
      return Array.isArray(records) ? records : [];
    }
    // Unversioned legacy shape: discard rather than risk a stale schema.
    return [];
  } catch {
    return [];
  }
...
async function writeHistory(records: ConnectionRecord[]): Promise<void> {
  const envelope: HistoryEnvelope = { version: HISTORY_VERSION, records };
